...
def ask_openrouter_question():
    ...
    data = {
        ...
        "messages": [
            {"role": "system", "content": "Ты — доброжелательный и понимающий собеседник, который задаёт девушке по имени Зарина по одному вопросу за раз. Вопросы должны быть на разные темы: семья, творчество, интересы, с юмором, философские и душевные. Зарина — моя девушка, у неё биополярное расстройство, поэтому вопросы должны быть лёгкими, поддерживающими и позитивными, чтобы поднимать настроение. Не задавай несколько вопросов сразу, только один вопрос. Не давай объяснений, просто сформулируй короткий и интересный вопрос.
."}
        ],
        "max_tokens": 100,
        "temperature": 0.7,
